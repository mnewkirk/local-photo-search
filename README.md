...
| **M18** | In progress | Unknown-face similarity stacking + merge suggestions. Two surfaces: (1) `recluster-faces` gained a session-stacking second pass that groups DBSCAN noise points by tight ArcFace similarity + time proximity, so lone unknowns from the same session get clustered instead of hidden. (2) New `suggest-face-merges` CLI (dry-run only) surfaces likely merges between any two groups — cluster↔cluster and cluster↔named — using centroid + min-pair L2 distance, with a `--verify-pair` harness for threshold tuning against known TP/FP examples. Accept/reject UI is pending. |
