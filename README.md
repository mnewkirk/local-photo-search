...
| **M18** | In progress | Unknown-face similarity stacking + merge suggestions. Three surfaces: (1) `recluster-faces` gained a session-stacking second pass that groups DBSCAN noise points by tight ArcFace similarity + time proximity. (2) `suggest-face-merges` CLI finds likely merges between any two groups (cluster↔cluster, cluster↔named) via centroid + min-pair L2 distance; `--verify-pair` harness tunes thresholds. (3) `/merges` review page renders each suggestion side-by-side with face crops, scores, and one-click accept/dismiss — reads the CLI's `--json-out` file, performs merges via `POST /api/faces/merges`. Remaining work: persistent rejection table + schema bump (dismissals are currently localStorage-only). |
| **M19** | Planned | Inferred geotagging — for photos missing GPS, infer location from nearby-in-time photos (within seconds/minutes) that do have GPS. May require ingesting smartphone photos from Google Photos to provide the GPS-bearing neighbors for camera photos that lack EXIF location. |
