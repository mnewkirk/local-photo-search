...
### Signal 4: Registered person names in the query

If the free-text query contains names of people who've been registered
(via `add-persons` / face references), those names are lifted out of the
query and applied as AND-intersected person filters. `?q=Calvin and Ellie`
returns only photos where both Calvin **and** Ellie appear. Matching is
case-insensitive, word-bounded, and longest-first (so "Matt Newkirk" beats
"Matt"), and connector tokens (`and`, `with`, `&`, `,`) are stripped from
whatever residual is passed to CLIP. Combined with the other signals:
`?q=Calvin and Ellie at the beach` → intersection of Calvin's photos,
Ellie's photos, and CLIP("at the beach"). Prefix with `-` to exclude
(e.g. `-Calvin`) — the lookbehind keeps the exclusion in the CLIP path
instead of treating it as a person filter.
...
# POST /api/stacks/detect               — run detection synchronously
# POST /api/stacks/detect/stream        — run detection with SSE progress + cancel
#                                          (powers the status-page Stacking form)
```

You can also run stacking from the **Status page** (`/status`) — the
Stacking form has editable `time_window_sec` / `clip_threshold` /
`max_stack_span_sec` parameters plus "Clear existing stacks first" and
"Dry run" toggles, a live progress bar (phase label, done/total, pair
count, elapsed seconds), and a Cancel button that aborts within ~1s even
on large libraries.
