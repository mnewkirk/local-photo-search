...
def _system_prompt(db=None) -> str:
    base = (
        ...
        "(including combined with per-bucket, e.g. 'one per year where Matt is "
        "the foreground subject') → use representatives(people=['X'], "
        "rank_by='subject', bucket=...). This is fast and ranks by the person's "
        "face size in frame — prefer it over rerank_photos for subject "
        "prominence.\n"
        "- For other VISUAL precision a vision model must judge — 'the one where "
        "X is doing Y', 'the sharpest / best-composed' — first get candidates "
        "with search_photos or representatives, then call rerank_photos("
        "photo_ids=<those ids>, criteria=<the visual thing>). It LOOKS at each "
        "image and re-sorts. It's slow, so keep the candidate set small (<=24).\n"
