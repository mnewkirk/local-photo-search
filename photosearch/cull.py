...
def load_selections(db: PhotoDB, directory: str) -> Optional[list[dict]]:
    ...
    rows = db.conn.execute(
        """SELECT rs.photo_id, rs.selected, rs.cluster_id, rs.rank_in_cluster,
                  ...
                  p.aes_overall_day_pct, p.aes_subject_overall_day_pct,
                  p.camera_make, p.camera_model, p.date_taken,
                  p.raw_filepath, p.place_name
           FROM review_selections rs
           JOIN photos p ON p.id = rs.photo_id
           WHERE rs.directory = ?
           ORDER BY rs.selected DESC,
                    COALESCE(p.aes_subject_overall_pct, p.aes_overall_pct, -1) DESC,
                    p.aesthetic_score DESC""",
        (directory,),
    ).fetchall()
